defmodule D10C2 do
  ...
  def navigate({x, y, from_dir, sym}, count, map, acc) do
    ...
    if next_symbol == "S" do
      ...
      navigate(
        {next_x, next_y, next_from, next_symbol},
        count + 1,
        map,
        acc ++ [{next_x, next_y}]
      )
  ...
  def clean_map(map, x_bound, y_bound, closed_list) do
    Enum.reduce(0..(y_bound - 1), map, fn y, map_acc ->
      ...
      new_row =
        Enum.reduce(0..(x_bound - 1), row, fn x, row_acc ->
  ...
  def get_symbol_from_neighbors([north, south, _, _])
      when north in @north_symbols and south in @south_symbols,
      do: "|"

  def get_symbol_from_neighbors([north, _, east, _])
      when north in @north_symbols and east in @east_symbols,
      do: "L"

  def get_symbol_from_neighbors([north, _, _, west])
      when north in @north_symbols and west in @west_symbols,
      do: "J"

  def get_symbol_from_neighbors([_, south, east, _])
      when south in @south_symbols and east in @east_symbols,
      do: "F"

  def get_symbol_from_neighbors([_, south, _, west])
      when south in @south_symbols and west in @west_symbols,
      do: "7"

  def get_symbol_from_neighbors([_, _, east, west])
      when east in @east_symbols and west in @west_symbols,
      do: "-"
