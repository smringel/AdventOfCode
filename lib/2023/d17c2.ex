defmodule D17C2 do
  ...
  def bfs(history, map, goal, closed_list) do
    ...
    if pos == goal and steps >= 4 do
      ...
      neighbors =
        ...
        |> Enum.flat_map(fn {dx, dy} = next_dir ->
          ...
          cond do
            ...
            dir == {-dx, -dy} -> []
            dir == next_dir and steps < 10 -> [{next_pos, steps + 1, dir}]
            dir == next_dir and steps >= 10 -> []
            steps >= 4 or steps == 0 -> [{next_pos, 1, next_dir}]
            true -> []
          end
        end)
      ...
      updated_closed_list =
        Enum.reduce(neighbors, closed_list, fn neighbor, acc ->
          MapSet.put(acc, neighbor)
        end)

      updated_history =
        Enum.reduce(neighbors, h, fn {{next_x, next_y}, _, _} = elem, acc ->
